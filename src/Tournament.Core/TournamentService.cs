...
namespace Tournament.Core
{
    public class TournamentService : ITournamentService
    {
        ...
        public async Task<bool> Add(Entities.Tournament tournament,string baseUrl)
        {
            ...
            if (result)
                backgroundJobClient.Schedule(() => ArrangeGames(tournament.Id,baseUrl), tournament.StartTime.AddHours(-3));
            return result;
        }
                ...
                else
                {
                    if (tournament.Games!= null)
                    {
                        ...
                        foreach (var game in tournament.Games)
                        {
                            ...
                            backgroundJobClient.Schedule(() => SendEmail(game.Player1.Email, gameReadySubject, body), tournament.StartTime.AddHours(-3));
                            backgroundJobClient.Schedule(() => SendEmail(game.Player2.Email, gameReadySubject, body), tournament.StartTime.AddHours(-3));
                        }
                    }
                ...
                if (!await tournamentRepository.IsUserAttendedToTournament(dto.TournamentId, dto.PlayerId))
                {
                    ...
                    if (user != null)
                    {
                        ...
                        if (tournament != null)
                        {
                            ...
                            if (result.Success)
                            {
                                backgroundJobClient.Schedule(() => SendEmail(user.Email, "Tournament is starting in 1 hour!", "Check out your email you will get an game url at start time."), tournament.StartTime.AddHours(-4));
